...
class HBNBCommand(cmd.Cmd):
    ...
    def __args_to_dict(self, args, class_name, **kwargs):
        ...
        for item in dummy:
            ...
            key = key.strip('"')
            value = value.strip('"')
            keys_list = list(storage.attributes()[class_name].keys())
            if not value or key not in keys_list:
                continue
    ...
    def help_create(self):
        ...
        print("Creates a class of any type where param key=value")
        print("[Usage]: create <className>")
        print("[Usage]: create <className> <param> <param> ...\n")
    ...
    def do_all(self, args):
        ...
        print_list = []
        if STORAGE_TYPE == "db":
            from_items = storage.all()
        else:
            from_items = storage._FileStorage__objects
        if args:
            args = args.split(' ')[0]  # remove possible trailing args
            if args not in HBNBCommand.classes:
                print("** class doesn't exist **")
                return
            for k, v in from_items.items():
                if k.split('.')[0] == args:
